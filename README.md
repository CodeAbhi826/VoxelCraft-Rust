...
**816 tests · clippy clean (0 warnings) · WGSL validated · E2E-screenshot verified**
...
| CI | 4 workflows: tests (816), wasm bundle auto-rebuild, single-file Linux build, releases |
...
- **816/816 tests green** (`cargo test --release --no-default-features --workspace`) — including WGSL parse+validation of every shader via naga, drift-guard tests for every historical bug fix (texture-seam quartet, flat water, FSR identity-at-1×, …), and per-subsystem constant checks against the wiki values
