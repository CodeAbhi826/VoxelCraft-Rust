...
| **Game Modes** | Survival, Creative, Adventure, Hardcore, Spectator | 5 modes defined in `vc-gameplay/src/modes.rs` | Hardcore world permadeath delete-world screen, Adventure block break tags (`CanDestroy`), Spectator mob spectating. |
| **UI, GUI Scale & FSR** | GUI Scale (Auto, 1x, 2x, 3x, 4x), FSR / Upscaling presets, Chat (T / /) | UI canvas 960×540, FSR EASU/RCAS in render.rs | Canonical FSR 1.0 UI presets (Ultra Quality, Quality, Balanced, Performance) + sharpness slider; integer GUI scaling without pixel jitter. |
