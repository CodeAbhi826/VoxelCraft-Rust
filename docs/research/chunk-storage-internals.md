# the reference game Java 1.16.5 Chunk Storage Internals
...
|---|---|---|
| Chunk column | 16 × 256 × 16 (blocks), 16 sections of 16³ | [Chunk format – Block format](https://the reference wiki w/Chunk_format) |
| Section size | 16×16×16 = 4096 blocks; sections Y=0..15; empty sections not saved | same |
| Build range (Overworld) | block Y 0..=255 ("build limit 256"); heightmap values 0..=256 | [Anvil history](https://the reference wiki w/Anvil_file_format), [protocol heightmap](https://the reference wiki w/Java_Edition_protocol/Chunk_format) |
| Global block state count | **17,112 states, 763 blocks** (registry unchanged 1.16.2→1.16.5) | [the reference game-data 1.16.2](https://github.com/PrismarineJS/the reference game-data/blob/master/data/pc/1.16.2/blocks.json) + [yarn 1.16.5 Blocks javadoc](https://maven.fabricmc.net/docs/yarn-1.16.5+build.10/net/the reference game/block/Blocks.html) |
| Global palette bits (blocks) | **ceil(log2(17112)) = 15 bits** (direct palette) | computed; consistent with [protocol](https://the reference wiki w/Java_Edition_protocol/Chunk_format) ("15 bits" for modern vanilla, ≥9 ⇒ global) |
...
| Palette index packing | `entries_per_long = floor(64/bits)`; `longs = ceil(4096/entries_per_long)`; **entries never straddle a long boundary** | [protocol Data Array format](https://the reference wiki w/Java_Edition_protocol/Chunk_format) |
| 4-bit section payload | 16 entries/long ⇒ **256 longs = 2048 bytes** | computed from rule above |
| 5-bit | 12/long ⇒ 342 longs (matches MC-239610 example) | [MC-239610](reference bug tracker MC-239610) |
| 6/7/8/15-bit | 410 / 456 / 512 / 1024 longs | computed |
| Index order in arrays | **YZX**: `i = (y<<8) | (z<<4) | x` ("letters=X, lines=Z, pages=Y") | [Chunk format – Block format](https://the reference wiki w/Chunk_format) |
| Block/sky light | **2048 bytes per section per type** (4096 nibbles); **even index = LOW nibble, odd = HIGH nibble** | [Chunk format](https://the reference wiki w/Chunk_format), [wiki.vg](https://c4k3.github.io/wiki.vg/Chunk_Format.html) |
| Biomes | **per column**: `Biomes` IntArray(256) at `Level` root; 3D 4×4×4 (64 entries/section) only since **1.18** | [Chunk format history](https://the reference wiki w/Chunk_format) |
| Biome count 1.16.5 | 79 registered biomes | [the reference game-data 1.16.2 biomes.json](https://github.com/PrismarineJS/the reference game-data/blob/master/data/pc/1.16.2/biomes.json) |
| Heightmaps | 9-bit values 0..=256, **7 per long, 37 longs** per map; 6 types | [Chunk format](https://the reference wiki w/Chunk_format), [Heightmap](https://the reference wiki w/Heightmap) |
| Data version | 1.16.5 = **2586** | [Data version list](https://the reference wiki w/Data_version), [misode versions](https://misode.github.io/versions) |
| Region file | 4 KiB sectors; 8 KiB header (1024×[3B offset+1B count] + 1024×4B timestamps); payload = 4B length + 1B compression + data; 1.16.5 saves **zlib (scheme 2)** | [Region file format](https://the reference wiki w/Region_file_format) |
| 1.16.5 palette classes | `ArrayPalette`, `HashMapPalette`, `IdentityPalette` (global) — **no singleton palette in 1.16.5** | [Forge 1.16.5 javadoc package](https://nekoyue.github.io/ForgeJavaDocs-NG/javadoc/1.16.5/net/the reference game/util/palette/package-summary.html) |
...
1.16.5 has exactly three palette implementations (publisher-mapped names, from the Forge 1.16.5
...
- bits = **15 (global, indices are raw state IDs)** if the palette would exceed 256 entries.
  ...
  ([Chunk format](https://the reference wiki w/Chunk_format)); the 256-entry threshold before going
...
- **No single-value palette in 1.16.5**: the class doesn't exist in 1.16.5 (only added in 1.18's
  ...
  [Chunk format](https://the reference wiki w/Chunk_format) page which describes 1.18+.
...
[Forge 1.16.5 PalettedContainer javadoc](https://nekoyue.github.io/ForgeJavaDocs-NG/javadoc/1.16.5/net/the reference game/util/palette/PalettedContainer.html);
yarn 1.16.5 mirrors it as `writeLock`
[yarn 1.16.5](https://maven.fabricmc.net/docs/yarn-1.16.5+build.10/net/the reference game/world/chunk/PalettedContainer.html)).
...
- `Palette` (list of compounds): each entry `{ Name: "the reference game:stone", Properties: { facing: "north", ... } }`
...
> [Chunk format](https://the reference wiki w/Chunk_format)
...
> — [Java Edition protocol/Chunk format](https://the reference wiki w/Java_Edition_protocol/Chunk_format)
...
[go-theft-craft chunk pkg](https://pkg.go.dev/github.com/go-theft-craft/the reference game-protocol/wire/java/chunk)).
...
blocks/long ⇒ 342 longs. reference bug tracker MC-239610)
...
- 3D biomes (quart resolution, 4×4×4 cells, 64 per section, paletted with 1–3 bit indirect /
  ...
  [protocol](https://the reference wiki w/Java_Edition_protocol/Chunk_format); Bedrock got per-block
...
- 1.16.5 has **79 registered biomes** (Overworld 61 + Nether 5 + End 5 + 8 technical/unused
  retained) — count from
  [the reference game-data 1.16.2 biomes.json](https://github.com/PrismarineJS/the reference game-data/blob/master/data/pc/1.16.2/biomes.json)
...
- Section = 16×16×16; column = 16 sections, `Y` byte 0 (bottom) .. 15 (top) in 1.16.5
  ...
  [Chunk format – Block format](https://the reference wiki w/Chunk_format)).
- Overworld block Y range: **0..=255** (the "256 build limit" — heightmaps can read 256 =
  ...
  [Chunk format](https://the reference wiki w/Chunk_format)). Anvil raised the build height to
  256 in 1.2.1 ([Anvil](https://the reference wiki w/Anvil_file_format)). Nether 0..=127
...
- **Per-section `BlockCount` / `nonEmptySectionCount` are runtime bookkeeping, not NBT**: the
  ...
  [Forge 1.16.5 ChunkSection javadoc](https://nekoyue.github.io/ForgeJavaDocs-NG/javadoc/1.16.5/net/the reference game/world/chunk/ChunkSection.html)),
...
- 1.16.5: **763 blocks / 17,112 block states** ⇒ **15 bits** global palette.
  ...
  [yarn 1.16.5 Blocks javadoc](https://maven.fabricmc.net/docs/yarn-1.16.5+build.10/net/the reference game/block/Blocks.html)
  and 763 blocks with 17,112 min→max state IDs in
  [the reference game-data 1.16.2 blocks.json](https://github.com/PrismarineJS/the reference game-data/blob/master/data/pc/1.16.2/blocks.json)
...
- The "~26,000 states" figure is a **modern** number, not 1.16.5. Measured from
  [the reference game-data](https://github.com/PrismarineJS/the reference game-data): 1.16.2=17,112;
  ...
  ([source](https://the reference wiki w/Java_Edition_protocol/Chunk_format)), so 15 bits has been
...
- Registry IDs are **signed ints in vanilla**, and the direct width can grow up to 31 bits with
  mods ([protocol note](https://the reference wiki w/Java_Edition_protocol/Chunk_format)).
...
- One ID per unique combination; "if a block has multiple properties then the number of
  ...
  [protocol](https://the reference wiki w/Java_Edition_protocol/Chunk_format).
- `state_id = block_base_id + combination_index` where the combination index is a **mixed-radix
  ...
  1. **Properties are sorted alphabetically by property NAME** (the 1.16.5
     ...
     [Forge 1.16.5 StateContainer javadoc](https://nekoyue.github.io/ForgeJavaDocs-NG/javadoc/1.16.5/net/the reference game/state/StateContainer.html));
  ...
  - Empirical validation (this research): using
    [the reference game-data 1.21.4 blocks.json](https://github.com/PrismarineJS/the reference game-data/blob/master/data/pc/1.21.4/blocks.json)
...
- Per section, per type: **2048 bytes** = 4096 nibbles, YZX order
  ...
  block" — [Chunk format](https://the reference wiki w/Chunk_format)).
- **Nibble packing (canonical)**: "Light level is stored as 4 bits per block, 2 blocks sharing
  ...
  ([Chunk format](https://the reference wiki w/Chunk_format)); the wiki.vg-era protocol text
...
Six types exist ([Heightmap](https://the reference wiki w/Heightmap)); the stored value is
...
([Heightmap](https://the reference wiki w/Heightmap); the protocol packet currently ships
...
[Chunk format](https://the reference wiki w/Chunk_format)). Index = `z*16+x`, x fastest.
...
([Region file format](https://the reference wiki w/Region_file_format)):
...
- Decompressed payload = **NBT** (gzip-independent; region-level compression only). Root
  ...
  ([Chunk format NBT structure](https://the reference wiki w/Chunk_format))
...
- Use `StateId = u32` for the registry, palettes, mesh vertex channels, and APIs. Vanilla
  ...
  [protocol](https://the reference wiki w/Java_Edition_protocol/Chunk_format)). The only place
...
- Chunk format (NBT, sections, palette rule, light, heightmaps, biome history):
  https://the reference wiki w/Chunk_format
- Protocol chunk format (packing formula, palette formats, heightmap bits formula, biome
  4×4×4, global palette 15 bits): https://the reference wiki w/Java_Edition_protocol/Chunk_format
- Region file format: https://the reference wiki w/Region_file_format
- Anvil file format (build height 256, YZX ordering): https://the reference wiki w/Anvil_file_format
- Heightmap types/semantics: https://the reference wiki w/Heightmap
- Data version list (1.16.5 = 2586): https://the reference wiki w/Data_version ,
...
- 1.16.5 code structure (PalettedContainer fields incl. ReentrantLock; palette classes
  ...
  https://nekoyue.github.io/ForgeJavaDocs-NG/javadoc/1.16.5/net/the reference game/util/palette/PalettedContainer.html
  .../util/palette/package-summary.html , .../state/StateContainer.html ,
  .../world/chunk/ChunkSection.html ,
  https://maven.fabricmc.net/docs/yarn-1.16.5+build.10/net/the reference game/world/chunk/PalettedContainer.html ,
  .../net/the reference game/block/Blocks.html (763 fields)
- Counts (763 blocks / 17,112 states 1.16.2–1.16.5; 79 biomes; per-version state totals):
  https://github.com/PrismarineJS/the reference game-data/blob/master/data/pc/1.16.2/blocks.json ,
  .../1.16.2/biomes.json , .../1.21.4/blocks.json (defaultState offsets used to verify the
  ID-assignment algorithm)
- Packing cross-check (342 longs @5 bits / 14-entry palette): reference bug tracker MC-239610
...
- "never straddle" (Go implementation phrasing):
  https://pkg.go.dev/github.com/go-theft-craft/the reference game-protocol/wire/java/chunk
