...
a placeholder, verify live (the reference wiki, the project's own generated
...
| Minecart max speed 8 blocks/second | ✅ use | Confirmed from the reference wiki's own Minecart page (0.4 blocks/tick cap). |
...
|---|---|---|
| Section 11 "Pointed Dripstone Fall Damage" — **the entire section** | ❌ delete outright | Pointed dripstone / dripstone caves / stalactites / stalagmites are Caves & Cliffs Part I content — **added in the reference game's 1.17**. None of it exists in 1.16.5. Not a "verify the formula" case: the mechanic itself is out of scope for this project's target version. No dripstone-related fall damage, ever. |
...
- Falling-block "fall delay" values (2/5 game ticks) — source does not clearly
  ...
  `FallingBlockEntity` behavior description on the reference wiki first.
...
- Nether biome mob spawn weights — community-compiled, explicitly not
  ...
  the reference wiki biome page before using specific numbers.
...
**That formula is wrong** — this project already debunked it via the original publisher's own
...
re-checked against the live wiki (the reference wiki). Outcomes:

| Claim | Outcome | Source (live) |
|---|---|---|
| Sprint-swimming 3.918 b/s | ✅ CONFIRMED — "Sprint-swimming: 3.918 m/s" | the reference wiki w/Transportation |
| Swimming speeds "downstream 1.81 / upstream 0.39" (Doc 1 labels) | ❌ MISLABELED — wiki table: still-water underwater 1.97 m/s, surface 2.20 m/s, upstream 1.81 m/s, diving 0.39 m/s, downstream-standing 1.37 m/s. The 1.81/0.39 numbers exist but with different meanings than the doc attached. Engine uses the verified set. | the reference wiki w/Transportation |
| Drowning: air supply, damage | ✅ CONFIRMED — air max 300 (decreases 1/game tick), 10 bubbles × 30 air (bubble pops at every multiple of 30 + 2); damage 2 HP per second taken when air reaches −20, air resets to 0 after damage; out of water air regenerates 1 bubble (30) per 4 game ticks; Respiration skip chance x/(x+1). | the reference wiki w/Damage §Drowning |
| Falling-block entity physics | ✅ NEW — wiki entity-physics table: gravity −0.04 m/tick² (HALF the 0.08 entity gravity), Drag-Y 0.98. Also: exists > 600 ticks → drops as item; lands on solid-top blocks; anvil deals 2 HP per block fallen after the first. | the reference wiki w/Falling_Block |
| Falling-block "fall delay" 2/5 game ticks (sand/anvil) | ⚠ STAYS UNVERIFIED — the wiki documents no per-block spawn delay. Engine keeps its documented block-wise approximation and cites the verified entity constants. | the reference wiki w/Falling_Block (absent) |
| Scaffolding 6-block limit | ✅ CONFIRMED — "Has a distance of at least 7 from the closest supported scaffolding" → falls. (6-block reach = falls at 7.) | the reference wiki w/Falling_Block |
| Villager gossip table | ✅ CONFIRMED (Java): trading gain 4 / decay 2 / share 20 / max 25 / mult 1; major_positive 20 / 0 / 100 / 20 / 5; minor_positive 25 / 1 / 5 / 25 / 1; minor_negative 25 / 20 / 20 / 200 / −1; major_negative 25 / 10 / 10 / 100 / −5. Kill shares to villagers with line-of-sight in a 16-block box; trade/cure/attack affect the target only; decay every 20 minutes; shared gossip reduced by sharing cost; major_positive unshareable (share 100 > max 20); reputation = Σ(value × multiplier); iron golems hostile at reputation ≤ −100. | the reference wiki w/Villager §Gossiping |
| Gossip → trade price | ✅ CONFIRMED — price decreased by floor(reputation × trade priceMultiplier); demand (when positive) increases price by floor(initial price × priceMultiplier × demand); clamped to [1, 64] and stack size. | the reference wiki w/Trading §Sale prices |
| Passive mob spawn cycle | ✅ CONFIRMED — passive mobs get ONE spawn cycle per 400 game ticks (20 s); most animals spawn within chunks when generated (initial spawn ignores mob cap); spawn requires a player horizontally within 128 blocks of the chunk center; hostile mobs > 128 blocks from nearest player despawn instantly. | the reference wiki w/Mob_spawning |
| Firework boost 33.5 b/s | ❌ CONTRADICTED — current wiki states 35.5 blocks/second (elytra firework propulsion). Neither 33.5 nor 35.5 is confirmed as the 1.16.5-exact value; elytra/fireworks are not in the engine — recorded here only. | the reference wiki w/Elytra §Powered flight |
| Minecart friction 0.01 / "empty decelerates faster" | ⚠ STAYS UNVERIFIED — current Minecart page documents the 8 b/s per-axis cap (✅, diagonal 11.314) but no friction coefficient. No minecarts in engine. | the reference wiki w/Minecart |
| Nether biome spawn weights (Doc 1 §4) | ⚠ STAYS UNVERIFIED as 1.16.5-exact — the current wiki documents the modern (post-1.18 world-gen) spawn tables, which do not match the doc's community-compiled numbers. No Nether natural spawning in engine; recorded with caveat. | the reference wiki w/Mob_spawning |
| Hopper container | ✅ CONFIRMED — 5 slots; GUI labeled "Item Hopper"; transfer cooldown 8 game ticks (2.5 items/s); opens on use; locked by redstone. GUI height 176×133 per this file's standing correction (wiki states no pixel dims). | the reference wiki w/Hopper §Container |
| F3 "Looking at Fluid" as separate line | ✅ CONFIRMED for 1.16.5 — split added 1.13 18w22c ("The debug overlay now shows the fluid the player is looking at, separately from blocks"). | the reference wiki w/Debug_screen §History |
| F3 core line formats | ✅ VERIFIED — XYZ / Block / Chunk [RX RZ in F] / Facing D (Towards T) (Y P) / Client Light L (S sky, B block) / Biome / Local Difficulty D // C / Targeted Block / Mem / Allocated formats confirmed. JVM-specific lines (Java Version, GPU, Mem) get engine-native adaptations (Rust + wgpu backend/adapter) — documented adaptation, not a 1:1 copy. | the reference wiki w/Debug_screen |
