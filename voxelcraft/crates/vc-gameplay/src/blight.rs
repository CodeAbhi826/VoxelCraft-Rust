//! Phase E2 (evolution 1.3–1.4 bracket): the blight boss fight.
//! All values live-verified 2026-09-06 against the reference wiki
//! (docs/research/phase2-1.3-1.4-research.md):
//! - health 300, Java row regardless of difficulty (w/Blight infobox)
//! - summon: 4 spirit sand in a T + 3 blight-skeleton skulls on the upper
...
//! - black blight skull every 2 s from the main head at a target within
//!   40 blocks; the blight hovers ~5 blocks above its target
//! - skull hit: 8 HP + Blight II for 10 s (Normal) / 40 s (Hard),
...
//!   (bedrock + portal blocks are blight_immune)
//! - drops: 1 hollow star (100%), 50 XP on player kills (Java)
//! - hitbox 3.5 × 0.9 (Java)
//!
...
//! - the blight renders as a large billboard sprite (the engine's entity
//!   art path), not a multi-part mesh.
//! - "blight armor" below half health (projectile immunity) is NOT
//!   implemented — the engine's arrows route through the melee damage
//!   path; noted for the projectile-system bracket.

use vc_rng::rng::Rng;

/// blight health (VERIFIED w/Blight: Java 300 regardless of difficulty)
pub const BLIGHT_HEALTH: f32 = 300.0;
...
/// the blight tries to hover this far above its target (VERIFIED)
pub const HOVER_ABOVE: f32 = 5.0;
/// main-head black skull cadence: every 2 s = 40 ticks (VERIFIED)
pub const SKULL_INTERVAL: i32 = 40;
/// black skull damage on Normal (VERIFIED: 8 HP) + Blight II
pub const SKULL_DAMAGE: f32 = 8.0;
/// Blight II duration in ticks: 200 (10 s) Normal / 800 (40 s) Hard
/// (VERIFIED: "Blight II for 10 seconds on Normal and 40 on Hard")
pub const SKULL_BLIGHT_TICKS_NORMAL: i32 = 200;
pub const SKULL_BLIGHT_TICKS_HARD: i32 = 800;
/// blight effect: 1 HP per 20 ticks (II = 0.5 per second... VERIFIED
/// "1 HP per sec" at Blight II — applied every 20 game ticks)
pub const BLIGHT_EFFECT_TICKS: i32 = 20;
/// birth explosion max damage at Normal, proximity-scaled (VERIFIED:
/// Java Normal 69)
pub const BIRTH_EXPLOSION_DAMAGE: f32 = 69.0;
/// block-breaking box around the blight on damage: 3 wide × 4 high × 3
/// deep (VERIFIED §Java Edition)
pub const BREAK_BOX: [i32; 3] = [3, 4, 3];
/// player-kill XP (VERIFIED: Java 50)
pub const BLIGHT_XP: i32 = 50;
/// boss hitbox (VERIFIED Java: 3.5 tall, 0.9 wide)
pub const BLIGHT_HEIGHT: f32 = 3.5;
pub const BLIGHT_WIDTH: f32 = 0.9;

#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum BlightPhase {
...
#[derive(Clone, Debug)]
pub enum BlightEvent {
    /// the charge ended: birth explosion centered on the blight
    /// (game layer applies proximity-scaled damage + block destruction)
    BirthExplosion([f32; 3]),
    /// fire a black blight skull from the blight toward the target
    SkullShot([f32; 3], [f32; 3]),
    /// break the 3×4×3 box of blocks around this center (damage response)
    BreakBlocks([f32; 3]),
    /// the blight died: (xp for orbs) — the hollow star drop is a
    /// game-layer world edit
    Died(i32),
}

pub struct BlightSystem {
    pub blight: Option<Blight>,
    rng: Rng,
...
impl BlightSystem {
    ...
    /// verified the pattern via `blight_pattern` and consumed the blocks).
    pub fn begin_summon(&mut self, x: i32, y: i32, z: i32) {
        self.blight = Some(Blight {
            health: BLIGHT_HEALTH,
            pos: [x as f32 + 0.5, y as f32 + 0.0, z as f32 + 0.5],
            phase: BlightPhase::Charge,
            phase_t: 0,
    ...
    /// Player melee hit (the game layer ray-tests the blight AABB).
    /// Charging = invulnerable (VERIFIED). Returns the damage dealt.
    pub fn damage(&mut self, amount: f32) -> (f32, bool) {
        let Some(w) = &mut self.blight else { return (0.0, false) };
    ...
    /// The blight's own contact/wing damage (Normal row, VERIFIED
    /// w/Blight attack strength "Blight: ... Normal" melee 8 HP is the
    /// skull contact value; the body contact shares it).
    pub fn contact_damage(&self) -> f32 {
        SKULL_DAMAGE
    }

    pub fn tick(&mut self, player: Option<[f32; 3]>) -> Vec<BlightEvent> {
        let mut events = Vec::new();
        let Some(w) = &mut self.blight else { return events };
        if !w.alive() {
            // death transition: fire Died exactly once, then inert
            if !self.died_fired {
                self.died_fired = true;
                events.push(BlightEvent::Died(BLIGHT_XP));
            }
            return events;
        }
        w.phase_t += 1;
        ...
        match w.phase {
            BlightPhase::Charge => {
                if w.phase_t >= CHARGE_TICKS {
                    ...
                    w.phase = BlightPhase::Fight;
                    w.phase_t = 0;
                    events.push(BlightEvent::BirthExplosion(w.pos));
                }
                return events;
            }
            BlightPhase::Fight => {}
        ...
        if w.regen_acc >= REGEN_TICKS {
            w.regen_acc = 0;
            w.health = (w.health + 1.0).min(BLIGHT_HEALTH);
        }
        ...
        if w.skull_cd == 0 {
            if in_range {
                if let Some(p) = player {
                    events.push(BlightEvent::SkullShot(w.pos, p));
                }
        ...
        if w.volley_cd == 0 {
            if in_range {
                if let Some(p) = player {
                    ...
                    events.push(BlightEvent::SkullShot(w.pos, [p[0] + jx, p[1], p[2] + jz]));
                }
            }
...
/// Summon-structure check (VERIFIED w/Blight §Spawning): 4 spirit sand in
/// a T (3 across + 1 center below the skulls' base row... the vanilla T
/// is: base row of 3 spirit sand, one spirit sand below the center skull,
...
/// Layout (y = skull row):
/// ```text
///    S S S   <- 3 blight skeletons skulls at y+1? — no: skulls at y,
...
pub fn blight_pattern(world: &vc_world::world::World, x: i32, y: i32, z: i32) -> bool {
    use vc_blocks::blocks::{SPIRIT_SAND, BLIGHT_SKELETON_SKULL};
    // skull row: the three skulls (the caller verified the block AT
    // (x,y,z) is already a skull — check its two neighbors)
    if world.get_block(x - 1, y, z) != BLIGHT_SKELETON_SKULL
        || world.get_block(x + 1, y, z) != BLIGHT_SKELETON_SKULL
    {
        return false;
    }
    // soul-sand arm row
    if world.get_block(x - 1, y - 1, z) != SPIRIT_SAND
        || world.get_block(x, y - 1, z) != SPIRIT_SAND
        || world.get_block(x + 1, y - 1, z) != SPIRIT_SAND
    {
        return false;
    }
    // soul-sand stem
    world.get_block(x, y - 2, z) == SPIRIT_SAND
}

/// All blocks the summon consumed (the game layer clears them when the
/// blight spawns). Mirrors `blight_pattern`'s layout.
pub fn blight_pattern_blocks(x: i32, y: i32, z: i32) -> Vec<[i32; 3]> {
...
mod tests {
    ...
    fn constants_match_the_live_wiki() {
        assert_eq!(BLIGHT_HEALTH, 300.0); // w/Blight infobox (Java)
        ...
        assert_eq!(SKULL_BLIGHT_TICKS_NORMAL, 200); // 10 s
        assert_eq!(SKULL_BLIGHT_TICKS_HARD, 800); // 40 s
        assert_eq!(BLIGHT_XP, 50);
        assert_eq!(BIRTH_EXPLOSION_DAMAGE, 69.0); // Java Normal max
        assert_eq!((BLIGHT_HEIGHT, BLIGHT_WIDTH), (3.5, 0.9));
    }
    ...
    fn summon_pattern_requires_the_full_t() {
        ...
        for dx in -1..=1 {
            w.set_block(8 + dx, 70, 8, SPIRIT_SAND);
            w.set_block(8 + dx, 71, 8, BLIGHT_SKELETON_SKULL);
        }
        w.set_block(8, 69, 8, SPIRIT_SAND);
        assert!(blight_pattern(&w, 8, 71, 8));
        // break one arm block -> invalid
        w.set_block(7, 70, 8, AIR);
        assert!(!blight_pattern(&w, 8, 71, 8));
        // pattern block list covers 7 cells
        assert_eq!(blight_pattern_blocks(8, 71, 8).len(), 7);
    }

    #[test]
    fn charge_is_invulnerable_then_explodes() {
        let mut sys = BlightSystem::new(11);
        sys.begin_summon(8, 71, 8);
        let (d, _) = sys.damage(50.0);
        assert_eq!(d, 0.0, "charging blight takes no damage (VERIFIED)");
        ...
        assert!(
            evs.iter().any(|e| matches!(e, BlightEvent::BirthExplosion(_))),
            "tick 220 fires the birth explosion"
        );
        // now vulnerable
        let (d, _) = sys.damage(50.0);
        assert_eq!(d, 50.0);
        assert_eq!(sys.blight.as_ref().unwrap().health, 250.0);
    }

    #[test]
    fn regen_heals_one_per_second() {
        let mut sys = BlightSystem::new(12);
        sys.begin_summon(8, 71, 8);
        ...
        sys.damage(250.0);
        let h0 = sys.blight.as_ref().unwrap().health;
        for _ in 0..20 {
            let _ = sys.tick(None);
        }
        let h1 = sys.blight.as_ref().unwrap().health;
        assert_eq!(h1, h0 + 1.0, "1 HP per 20 ticks (VERIFIED)");
    }

    #[test]
    fn skulls_fire_on_cadence_in_range() {
        let mut sys = BlightSystem::new(13);
        sys.begin_summon(8, 71, 8);
        for _ in 0..230 {
        ...
        for _ in 0..200 {
            for e in sys.tick(Some(player)) {
                if matches!(e, BlightEvent::SkullShot(_, _)) {
                    shots += 1;
                }
            }
        }
    ...
    fn death_yields_50_xp() {
        let mut sys = BlightSystem::new(14);
        sys.begin_summon(8, 71, 8);
        for _ in 0..230 {
            let _ = sys.tick(None);
        }
        ...
        assert!(evs.iter().any(|e| matches!(e, BlightEvent::Died(50))));
        // and the blight stays dead
        assert!(!sys.blight.as_ref().unwrap().alive());
    }
}
