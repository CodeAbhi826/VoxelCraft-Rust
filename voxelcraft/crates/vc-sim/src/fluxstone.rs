//! Fluxstone core (§25 subset, Phase 6): lever sources, wire power levels
//! with 1-per-block decay, and the inverting fluxstone torch — driven by
//! the deterministic scheduled-tick backbone (2 game ticks = 1 fluxstone
//! tick, vanilla timing).
//!
...
/// 1 fluxstone tick = 2 game ticks (vanilla)
pub const FLUXSTONE_TICK_RATE: u64 = 2;

/// true if a position holds any fluxstone component (wire/torch/lever +
/// the Phase 3 set)
#[inline]
pub fn is_component(s: u16) -> bool {
    ...
    b == FLUXSTONE_WIRE
        ...
        || b == OBSERVER
        || b == HOPPER
        // Phase E1: the fluxstone lamp re-checks its inputs on neighbor
        ...
        // 1.16 (Hollows Update, part 1): the target's POWER state feeds
        // wires — a neighbor change re-checks it, and its decay tick
        // rides the scheduler
        || b == TARGET
}

/// schedule fluxstone updates for a position and its neighbors after a
...
pub fn on_block_changed(sched: &mut TickScheduler, world: &World, x: i32, y: i32, z: i32) {
    for (dx, dy, dz) in [
        ...
        if state_block(ns) == REPEATER {
            ...
            sched.schedule([nx, ny, nz], FLUXSTONE_TICK_RATE);
        }
        // a torch ALSO re-checks when its support block changes
        if world.get_block(nx, ny + 1, nz) == FLUXSTONE_TORCH {
            sched.schedule([nx, ny + 1, nz], FLUXSTONE_TICK_RATE);
        }
    }
    // if the changed position is a torch, its own support may matter
    if world.get_block(x, y, z) == FLUXSTONE_TORCH {
        sched.schedule([x, y, z], FLUXSTONE_TICK_RATE);
    }
    ...
    for (sx, sy, sz) in [
        ...
        if matches!(sb_, PISTON | STICKY_PISTON | DISPENSER | DROPPER) {
            sched.schedule([sx, sy, sz], FLUXSTONE_TICK_RATE);
        }
    }
}
...
fn direct_feed(world: &World, x: i32, y: i32, z: i32) -> u8 {
    ...
    for (dx, dy, dz) in [
        ...
        if b == FLUXSTONE_TORCH && torch_is_lit(s) {
            best = best.max(15);
        }
        if b == FLUXSTONE_BLOCK {
            best = best.max(15);
        }
        ...
        if b == DAYLIGHT_SENSOR {
            let p = daylight_sensor_power(s) as u8;
            if p > 0 {
                best = best.max(p);
            }
        }
        ...
        // 1.16 (Hollows Update, part 1): the target's hit POWER state
        // feeds adjacent wire (VERIFIED w/Target: "produces a temporary
        // fluxstone signal when hit by a projectile ... strength 1–15 by
        // proximity"); a charged rebirth anchor's comparator signal =
        // its charge count (VERIFIED w/Rebirth_Anchor — the E3
...
pub fn wire_tick(world: &mut World, sched: &mut TickScheduler, x: i32, y: i32, z: i32) {
    ...
    for (dx, dy, dz) in [
        ...
        let p = wire_power(world.get_state(x + dx, y + dy, z + dz)) as u8;
        if p != 255 {
            target = target.max(p.saturating_sub(1));
        }
    }
    let old = wire_power(s) as u8;
    if target != old {
        world.set_block_state(x, y, z, wire_state(target));
        // notify: adjacent components re-check next fluxstone tick
        on_block_changed(sched, world, x, y, z);
        // torches AROUND a powered-changing wire re-check (their support
        // may be this wire)
        for (dx, dz) in [(1i32, 0i32), (-1, 0), (0, 1), (0, -1)] {
            let (wx, wz) = (x + dx, z + dz);
            if world.get_block(wx, y + 1, wz) == FLUXSTONE_TORCH {
                sched.schedule([wx, y + 1, wz], FLUXSTONE_TICK_RATE);
            }
            if world.get_block(wx, y - 1, wz) == FLUXSTONE_TORCH {
...
pub fn torch_tick(world: &mut World, sched: &mut TickScheduler, x: i32, y: i32, z: i32) {
    let s = world.get_state(x, y, z);
    if state_block(s) != FLUXSTONE_TORCH {
        return; // stale entry
    }
...
/// Phase E1: one fluxstone-lamp update (VERIFIED w/Fluxstone_Lamp):
...
///   our fluxstone backbone ticks at 2-game-tick granularity, so the
///   delayed OFF lands at the next fluxstone tick — a documented
...
pub fn lamp_tick(world: &mut World, sched: &mut TickScheduler, x: i32, y: i32, z: i32) {
    let s = world.get_state(x, y, z);
    if state_block(s) != FLUXSTONE_LAMP {
        return; // stale entry
    }
    let powered = lamp_powered(world, x, y, z);
    let lit = s == FLUXSTONE_LAMP_LIT;
    if powered && !lit {
        // ON (VERIFIED: "A fluxstone lamp activates instantly")
        world.set_block_state(x, y, z, FLUXSTONE_LAMP_LIT);
        on_block_changed(sched, world, x, y, z);
    } else if !powered && lit {
        // OFF at the next fluxstone tick (the ~4gt approximation above)
        world.set_block_state(x, y, z, FLUXSTONE_LAMP_STATE);
        on_block_changed(sched, world, x, y, z);
    }
}
...
// 2026-09-06: reference wiki /Block_of_Fluxstone, /w/Daylight_Detector,
...
/// Daylight_Detector §Fluxstone_component) — the engine adaptation maps
...
/// 1.16 (Hollows Update, part 1): target decay — the hit's POWER state
/// drops back to 0 when its pulse window ends (VERIFIED w/Target: "the
/// target emits fluxstone power for 8 game ticks ... Arrows and tridents
...
pub fn weighted_plate_signal(block: u16, entities: usize) -> u8 {
    if entities == 0 {
        return 0;
    }
    ...
    for (dx, dy, dz) in [
        ...
        if old != 255 {
            ...
            if signal == 0 && old != 0 {
                ...
                sched.schedule([wx, wy, wz], FLUXSTONE_TICK_RATE);
            } else if target != old {
...
mod tests {
    ...
    fn drain(world: &mut World, sched: &mut TickScheduler, max_ticks: u64) {
        for _ in 0..max_ticks {
            ...
            for pos in due {
                ...
                match b {
                    FLUXSTONE_WIRE => wire_tick(world, sched, pos[0], pos[1], pos[2]),
                    FLUXSTONE_TORCH => torch_tick(world, sched, pos[0], pos[1], pos[2]),
                    LEVER => lever_tick(world, pos[0], pos[1], pos[2]),
                    FLUXSTONE_LAMP => lamp_tick(world, sched, pos[0], pos[1], pos[2]),
    ...
    fn lever_powers_wire_chain_with_decay() {
        ...
        for x in 1..=15i32 {
            ...
            assert_eq!(p, (16 - x) as u16, "wire at x={x} power {p}");
        }
    ...
    fn torch_clock_oscillates() {
        ...
        // run 400 game ticks (20 s of fluxstone time) WITHOUT settling —
        ...
        for _ in 0..400 {
            ...
            for pos in due {
                ...
                match b {
                    FLUXSTONE_WIRE => wire_tick(&mut w, &mut sched, pos[0], pos[1], pos[2]),
                    FLUXSTONE_TORCH => torch_tick(&mut w, &mut sched, pos[0], pos[1], pos[2]),
                    LEVER => lever_tick(&w, pos[0], pos[1], pos[2]),
                    FLUXSTONE_LAMP => lamp_tick(&mut w, &mut sched, pos[0], pos[1], pos[2]),
                    _ => {}
                }
            }
...
fn power_at(world: &World, x: i32, y: i32, z: i32) -> u8 {
    ...
    for (dx, dy, dz) in [
        ...
        match b {
            FLUXSTONE_WIRE => {
                let p = wire_power(s) as u8;
                if p != 255 {
                    best = best.max(p);
                }
            }
            FLUXSTONE_BLOCK => {
                // Phase E3 (VERIFIED w/Block_of_Fluxstone): always-on
...
fn signal_at(world: &World, x: i32, y: i32, z: i32) -> u8 {
    ...
    let b = state_block(s);
    let mut v = match b {
        FLUXSTONE_WIRE => wire_power(s) as u8,
        LEVER => {
            if lever_is_on(s) {
                15
            } else {
...
pub fn piston_breaks(b: u16) -> bool {
    is_cross(b)
}

/// one repeater update. VERIFIED: delay 1..4 fluxstone ticks (scheduled
...
pub fn repeater_tick(world: &mut World, sched: &mut TickScheduler, x: i32, y: i32, z: i32) {
    ...
    if want != powered {
        ...
        // repeater is scheduled at its OWN delay: 1..4 fluxstone ticks =
...
/// hopper update gate: enabled unless powered (VERIFIED: fluxstone locks
...
pub fn hopper_enabled(world: &World, x: i32, y: i32, z: i32) -> bool {
    ...
    // VERIFIED: fluxstone power locks hoppers (enabled=false). Derived
...
mod phase3_tests {
    ...
    /// after its 1..4 fluxstone-tick delay
    #[test]
...
mod e1_lamp_tests {
    ...
    fn drain(w: &mut World, sched: &mut TickScheduler, max: u64) {
        for _ in 0..max {
            ...
            for pos in due {
                let b = state_block(w.get_state(pos[0], pos[1], pos[2]));
                match b {
                    FLUXSTONE_WIRE => wire_tick(w, sched, pos[0], pos[1], pos[2]),
                    FLUXSTONE_TORCH => torch_tick(w, sched, pos[0], pos[1], pos[2]),
                    LEVER => lever_tick(w, pos[0], pos[1], pos[2]),
                    FLUXSTONE_LAMP => lamp_tick(w, sched, pos[0], pos[1], pos[2]),
    ...
    /// Phase E1 (VERIFIED w/Fluxstone_Lamp): the lamp lights next to an
    /// on lever, and goes dark after the lever turns off (our fluxstone
    ...
    fn lamp_lights_and_turns_off_with_the_lever() {
        ...
        w.set_block_state(0, 65, 0, lever_state(true));
        w.set_block_state(1, 65, 0, FLUXSTONE_LAMP_STATE);
        on_block_changed(&mut sched, &w, 0, 65, 0);
        ...
        // lever OFF → the lamp goes dark on the next fluxstone tick
        let _ = toggle_lever(&mut w, &mut sched, 0, 65, 0);
        drain(&mut w, &mut sched, 50);
        assert_eq!(
            w.get_state(1, 65, 0),
            FLUXSTONE_LAMP_STATE,
    ...
    #[test]
    fn phase_e3_fluxstone_block_powers_adjacent_wire() {
        // VERIFIED w/Block_of_Fluxstone: "acts as a permanently powered
        // fluxstone power source ... provide weak power to their direct
        // neighbors at signal strength 15"
        let mut w = flat_world();
        let mut sched = TickScheduler::new();
        w.set_block_state(0, 65, 0, FLUXSTONE_BLOCK_STATE);
        w.set_block_state(1, 65, 0, wire_state(0));
        on_block_changed(&mut sched, &w, 0, 65, 0);
        drain(&mut w, &mut sched, 50);
    ...
    fn v112_glazed_pushable_but_not_pullable() {
        ...
        w.set_block_state(0, 66, 0, lever_state(true));
        crate::fluxstone::on_block_changed(&mut sched, &w, 0, 66, 0);
        for _ in 0..20 {
            let due = sched.tick();
            for pos in due {
                let b = state_block(w.get_state(pos[0], pos[1], pos[2]));
    ...
    /// 1.16 (Hollows Update, part 1): a projectile hit on a TARGET writes
    ...
    /// 1.16: a charged rebirth anchor feeds adjacent wire at its charge
    /// count (VERIFIED w/Rebirth_Anchor — the comparator-signal class,
