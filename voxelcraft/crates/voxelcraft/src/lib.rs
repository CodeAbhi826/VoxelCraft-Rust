...
/// The Classic Art builtin pack, baked into the native binary (the
/// vanilla "pre-1.14 textures" analog — clean-room look-alikes generated
/// by scripts/gen_classic_art.py). Preferred at runtime as the
/// extracted `builtin-packs/classic-art/` folder; this embedded table
/// is the single-file fallback. Wasm fetches it from
/// `/voxelcraft-pack-classic-art/` instead (pack.rs).
#[cfg(not(target_arch = "wasm32"))]
pub mod embedded_classic_art {
    include!(concat!(env!("OUT_DIR"), "/embedded_classic_art.rs"));
}
pub mod game;
pub mod player;
